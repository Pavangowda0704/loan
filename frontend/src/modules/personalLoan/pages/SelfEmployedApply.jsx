// ============================================================
//  SelfEmployedApply.jsx — Multi-step Self-Employed Apply
//  Route: /personal-loan/self-employed/apply
// ============================================================
...
const STEPS = ["Personal Details", "Business Details", "Loan Details", "Review & Submit"];

const initialForm = {
  full_name: "", phone: "", email: "", dob: "", pan_number: "", city: "",
  employment_type: "Self-Employed", company_name: "", monthly_income: "",
  work_experience: "", existing_emi: "",
  loan_product: "Self-Employed Personal Loan", loan_amount: "", tenure: "36", loan_purpose: "Business",
};

export default function SelfEmployedApply() {
  ...
  const validate = () => {
    ...
    if (step === 2) {
      if (!form.company_name || !form.monthly_income) return alert("Fill business name and income"), false;
    }
    if (step === 3) {
  ...
  const reviewRows = [
    ...
    ["Business Experience", form.work_experience || "—"],
  ...
  return (
    <div className="pl-new-page">
      <div className="pl-breadcrumb">
        ...
        <Link to="/personal-loan/self-employed">Self-Employed Loan</Link> <span>›</span>
        <span>Apply</span>
      </div>

      <div className="pl-apply-wrapper">
        <div className="pl-apply-header">
          <span className="pl-tag pl-tag--green">SELF-EMPLOYED PERSONAL LOAN</span>
          <h1>Apply for Personal Loan</h1>
          <p>Designed for business owners and self-employed professionals</p>
        </div>
        ...
        <div className="pl-stepper-new">
          {STEPS.map((s, i) => (
            <div key={s} className={`pl-step-new${step > i+1 ? " done" : step === i+1 ? " active" : ""}`}>
              <div className="pl-step-circle pl-step-circle--green">{step > i+1 ? "✓" : i+1}</div>
              <span>{s}</span>
        ...
        <div className="pl-apply-card-new">
          ...
          {step === 2 && (
            <>
              ...
              <div className="pl-form-grid">
                <input name="company_name" placeholder="Business / Company Name *" value={form.company_name} onChange={handleChange} />
                <input name="monthly_income" type="number" placeholder="Monthly Net Income (₹) *" value={form.monthly_income} onChange={handleChange} />
                <input name="work_experience" placeholder="Business Vintage (e.g. 3 years)" value={form.work_experience} onChange={handleChange} />
            ...
            <>
              ...
              <div className="pl-form-grid">
                ...
                <select name="loan_purpose" value={form.loan_purpose} onChange={handleChange}>
                  {["Business","Medical","Education","Travel","Marriage","Home Renovation","Other"].map(p => <option key={p}>{p}</option>)}
                </select>
              </div>
            </>
          )}
          ...
          <div className="pl-form-actions">
            ...
            {step < 4
              ? <button className="pl-primary-btn pl-primary-btn--green" onClick={next}>Next Step →</button>
              : <button className="pl-primary-btn pl-primary-btn--green" onClick={submit} disabled={submitting}>
